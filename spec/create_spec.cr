...
private def addslots(node, slot)
  Rcm::Command::Addslots.new(addr(node), slot)
end

private def meet(src, dst)
  Rcm::Command::Meet.new(addr(src), addr(dst))
end

private def replicate(src, dst)
  Rcm::Command::Replicate.new(addr(src), addr(dst))
end
